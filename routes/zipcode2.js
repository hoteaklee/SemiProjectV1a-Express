...
router.get('/gugun/:sido', async (req,res)=>{
    ...
    res.send(JSON.stringify( await guguns));    // 조회결과를 JSON형식으로 전송
});
...
router.get('/dong/:sido/:gugun', async (req,res)=>{
    ...
    res.send(JSON.stringify( await dongs));    // 조회결과를 JSON형식으로 전송
});
...
router.get('/zip/:sido/:gugun/:dong', async (req,res)=>{
    ...
    res.send(JSON.stringify( await zips));    // 조회결과를 JSON형식으로 전송
});
